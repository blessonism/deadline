...
const getDefaultTimer = () => {
  // 创建今天18:30的时间对象
  const now = new Date();
  const todayAt1830 = new Date(now);
  todayAt1830.setHours(18, 30, 0, 0);
  
  // 如果当前时间已经过了今天的18:30，则设置为明天的18:30
  if (now >= todayAt1830) {
    todayAt1830.setDate(todayAt1830.getDate() + 1);
  }
...
export function TimerProvider({ children }) {
  ...
  useEffect(() => {
    ...
    // 特殊处理：检测并删除建军节计时器
    const hasArmyDayTimer = loadedTimers.some(timer => 
      timer.name && (timer.name.includes('建军节') || timer.name.includes('军节'))
    );
    
    // 如果发现建军节计时器，或者没有计时器，添加默认计时器(下班时间)
    if (hasArmyDayTimer || loadedTimers.length === 0) {
      console.log('检测到建军节计时器或无计时器，重置为下班时间计时器');
      // 过滤掉所有自动生成的计时器
      const userTimers = loadedTimers.filter(timer => !timer.isAutoGenerated);
      const defaultTimer = getDefaultTimer();
      const newTimers = [defaultTimer, ...userTimers];
  ...
  // 检查并更新过期的默认计时器 - 现在主要用于每天更新下班时间计时器
  const checkAndUpdateDefaultTimer = () => {
    ...
    if (defaultTimer) {
      ...
      if (targetDate <= now) {
        ...
        console.log(`默认计时器 ${defaultTimer.name} 已过期，已更新为下一个下班时间 - ${new Date().toLocaleString()}`);
      }
    }
  };
